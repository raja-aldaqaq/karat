    ...
    if product.quantity_available <= 0:
        ...
        if len(cart) > 0:
            ...
            if cart[0].shop.id == product.shop.id:
                ...
                messages.warning(
                    request, "All items in the cart should be from the same shop.")
        else:
...
def create_order(user, shop):
    ...
    existing_order = Order.objects.filter(
        user=user, shop=shop, ordered=False).first()
    if not existing_order:
        # If no existing order, create a new one
        Order.objects.create(user=user, shop=shop,
                             ordered=False, total_amount=0)
